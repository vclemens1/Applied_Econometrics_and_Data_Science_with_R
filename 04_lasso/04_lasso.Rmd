...
We saw earlier, that the strength of our regularization depends on the parameter $\lambda$. Choosing the optimal value for this parameter is crucial, because it balances the trade-off between model complexity and prediction accuracy.
...
**Why evaluate at all?** A model isn't *good* because it fits the training data. It's good if it *generalizes* and supports the decision you care about. Evaluation tells us if we're actually meeting that goal, that means if the model predicts the outcome well.
...
2.  **RMSE** gives a sense of how far the model's predictions are from the true values, on average. In LASSO regression, since it imposes penalties on large coefficients. The lower the RMSE, the better the model's predictions. The RMSE is expressed in the same unit as the dependent variable. The formula for RMSE is:
...
# Ensure outcome is numeric and predictors are numeric or factors
# caret::train with glmnet will automatically create dummy variables for factors
train_control <- trainControl(method = "cv", number = 10)

# Train LASSO model predicting mathenote
lasso_model <- train(
  mathenote ~ ., 
  data = trainData,
  method = "glmnet",
...
# Extract LASSO-selected variables (non-zero coefficients at best lambda)
lasso_coefs <- coef(lasso_model$finalModel, s = lasso_model$bestTune$lambda)
lasso_selected <- rownames(lasso_coefs)[lasso_coefs[,1] != 0]
...
ols_formula <- as.formula(
  paste("mathenote ~", paste(names(trainData)[names(trainData) != "mathenote"], collapse = " + "))
)
...
# Extract statistically significant variables at 5% level
ols_significant <- names(coef(ols_model))[which(summary(ols_model)$coefficients[,4] < 0.05)]
ols_significant <- ols_significant[ols_significant != "(Intercept)"]
cat("Significant variables in OLS:\n")
print(ols_significant)

# -------------------------
# 3. Compare LASSO vs OLS
# -------------------------

overlap_vars <- intersect(lasso_selected, ols_significant)
cat("Variables selected by LASSO AND significant in OLS:\n")
print(overlap_vars)

# -------------------------
# 4. Optional: Visual Comparison (tidyverse version)
# -------------------------

library(tidyr)  # make sure tidyr is loaded

# Extract OLS coefficients (exclude intercept)
...
coef_df <- data.frame(
  ...
  OLS = ols_coefs,
  LASSO = as.numeric(lasso_coefs[names(ols_coefs), 1])
)

# Replace NA (for variables dropped by LASSO) with 0
coef_df$LASSO[is.na(coef_df$LASSO)] <- 0
...
# Plot comparison
ggplot(coef_long, aes(x = reorder(Variable, Coefficient), y = Coefficient, fill = Model)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +
  labs(title = "Comparison of OLS vs LASSO Coefficients",
