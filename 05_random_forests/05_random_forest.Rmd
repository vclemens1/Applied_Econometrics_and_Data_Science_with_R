...
We don't see a very clear pattern, but after about 20 trees, the prediction error does not change much anymore. Notice, that the value range on the y-axis is very narrow.

Now, we select a sufficient number of tree and assume `ntree`$=20$ is fixed. Using again the cross-validation approach included in the `caret` package, we tune the remaining parameter `mtry` and `min.node.size`. 
...
tune_grid <- expand.grid(
  mtry = seq(2,5,1),  # Number of variables randomly sampled at each split
...
tune_grid_2 <- expand.grid(
  mtry = seq(2,5,1),  # Number of variables randomly sampled at each split
  splitrule = "variance", # Number of trees in the forest
  min.node.size = c(5, 10, 15) # Minimum size of terminal nodes
)

set.seed(123) # Set seed for reproducibility 
rf_model <- train(
  statistiknote_w1 ~ female + risk_affine + fachsemester + 
                erstfach_name + alkohol, 
  data = trainData, 
  method = "ranger",             
  trControl = train_control, 
...
#2) What measure can we use to evaluate the performance?

rf_prob_pred <- predict(rf_model, data = testData)
RMSE(rf_prob_pred, testData$statistiknote_w1)
